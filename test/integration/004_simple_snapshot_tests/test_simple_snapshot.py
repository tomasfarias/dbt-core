...
class TestSimpleColumnSnapshotFiles(DBTIntegrationTest):
    ...
    def _run_snapshot_test(self):
        ...
        results = self.run_sql('select * from {}.{}.my_snapshot where last_name is not NULL'.format(database, self.unique_schema()),fetch='all')
        self.assertEqual(len(results), 3)
...
# test_basic.py  TODO: not converted
